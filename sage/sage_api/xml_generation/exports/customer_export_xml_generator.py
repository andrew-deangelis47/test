...
class CustomerExportXmlGenerator:
    ...
    TAG_POOL_ID = '<poolId xsi:type="xsd:string"/>'
    ...
    def generate_customer_creation_xml(self, i_file):
        req_xml = """"""
        ...
        req_xml += self._generate_input_xml_for_cust_creation(i_file)
    ...
    def generate_part_creation_xml(self, i_file):
        req_xml = """"""
        req_xml += self.SOAP_TAG_ENVELOPE_OPEN
        req_xml += self.SOAP_TAG_HEADER
        req_xml += self.SOAP_TAG_BODY_OPEN
        ...
        req_xml += self.TAG_INPUT_XML_OPEN
        req_xml += self.SAGE_API_C_DATA_TAG_OPEN
        req_xml += self._generate_input_xml_for_part_creation(i_file)
        req_xml += self.SAGE_API_C_DATA_CLOSE
        req_xml += self.SAGE_API_TAG_INPUT_XML_CLOSE
        req_xml += self.XML_TAG_WSS_RUN_CLOSE
        req_xml += self.SAGE_API_SOAP_TAG_BODY_CLOSE
    ...
    def _generate_input_xml_for_cust_creation(self, i_file: str):
        dictionary = {
            self.INPUT_XML_KEY_GRP1: {
                self.INPUT_XML_KEY_TEMPLATE_IMPORT: "YBPCPPEXP",
    ...
    def _generate_input_xml_for_part_creation(self, i_file: str):
        dictionary = {
            self.INPUT_XML_KEY_GRP1: {
                self.INPUT_XML_KEY_TEMPLATE_IMPORT: "ZITMPP",
                self.INPUT_XML_KEY_I_AOWSTA: "NO",
                self.INPUT_KEY_I_EXEC: "REALTIME",
                self.INPUT_XML_KEY_RECORD_SEPERATOR: "|",
                self.INPUT_XML_KEY_I_FILE: i_file
