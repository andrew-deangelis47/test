...
from baseintegration.utils import address_tokenize, tokenize, set_blank_to_default_value
...
def get_or_create_customer(
    ...
    if customer is None:
        customer = filter_exact_customer_id_by_name(name)
    if customer is not None:
...
def get_or_create_address(
    ...
    address = Address(
        ...
        line1=addr_dict.get('address1')[:45] if addr_dict.get('address1', None) is not None else None,
        line2=addr_dict.get('address2')[:45] if addr_dict.get('address2', None) is not None else None,
        city=addr_dict.get('city')[:20] if addr_dict.get('city', None) is not None else None,
        state=addr_dict.get('state')[:3] if addr_dict.get('state', None) is not None else None,
        zip=addr_dict.get('postal_code')[:10] if addr_dict.get('postal_code', None) is not None else None,
...
def filter_exact_customer_id_by_name(name):
    """This function is a workaround in case the Paperless ERP code is not set and Paperless account names equal JobBOSS customer codes"""
    return Customer.objects.filter(customer=name).first()
...
def increment_code(code, max_code_length=MAX_CODE_LENGTH):
    search = re.search(r'(\d+)$', code)  # noqa: W605
    if search is None:
...
def match_address(customer: Customer, addr_dict: dict) -> Address:  # noqa: C901
    """
    ...
    for candidate_address in qs.all():
        ...
        if len(addr_dict.get('postal_code', '')) == 5:
            if addr_dict.get('postal_code') != set_blank_to_default_value(candidate_address.zip, '')[0:5]:
                continue
        elif len(addr_dict.get('postal_code', '')) == 10:
            if len(set_blank_to_default_value(candidate_address.zip, '')) == 5:
...
def get_available_address_code(customer: Customer, ship=True):
    ...
    while tries < MAX_CODE_RETRIES:
        ...
        tries += 1
    logger.info(f"Can't find an unused address code for {customer.customer}")
    return None
