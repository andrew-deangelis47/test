...
class AutoNumberMixin(models.Model):
    ...
    def save_with_autonumber(self, prefix=None, suffix=None, *args, **kwargs):
        ...
        with transaction.atomic():
            ...
            for auto_type in self.auto_number_attrs:
                if not getattr(self, auto_type.attr_name):
                    auto_type.set_column(self, prefix=prefix, suffix=suffix)
            new_kwargs = kwargs
            if 'prefix' in new_kwargs:
                del new_kwargs['prefix']
            if 'suffix' in new_kwargs:
                del new_kwargs['suffix']
            self.save(*args, **new_kwargs)
...
class AutoColumnType:
    ...
    def set_column(self, record, prefix=None, suffix=None):
...
class AutoNumberColumn(AutoColumnType):
    ...
    def set_column(self, record, prefix=None, suffix=None):
        ...
        value = str(int(an_row.last_nbr) + 1)
        if prefix is not None:
            value = prefix + value
        if suffix is not None:
            value = value + suffix
        setattr(record, self.attr_name, value)
